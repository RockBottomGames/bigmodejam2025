...
func _on_enter_state(_prev_state: State) -> void:
	...
	spring_node_data.state_container_node.on_enter_lerp_to_unhooked_state.emit()
...
func _state_postprocess(delta: float) -> void:
	...
	spring_node_data.state_container_node.postprocess_lerp_to_unhooked_state.emit(delta, _lerp_time)

func _on_exit_state():
	spring_node_data.state_container_node.on_exit_lerp_to_unhooked_state.emit()
